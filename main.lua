local function MyAddonCommands(msg, editbox)
    print("|cFF00FF00[MythicSpamBlocker]: |cFFFFFFFF version 1.1 https://www.curseforge.com/wow/addons/mythicspamblocker")    
end
...
local function filterChat(self, event, text, playerName, languageName, channelName, playerName2, sepcialFlags, zoneChannelId, channelIndex, channelBaseName, _, lineId, guid, bnSenderId, isMoble) 
    ...
    if(channelName == "2. Trade - City") then
        local wts = 'wts'
        local keywords = {'gold only', 'power leveling', 'powerleveling', 'm%+', 'mythic', 'carry', '10/15/20', 'gold only%!', 'boosting', 'any key', 'guarantee time', 'pwrlvl',
        'nathria', 'torghast', 'layer', 'carries', 'carry', 'keys', 'twisting', 'corridors', 'castle', 'castle nathria', 'aotc' }
